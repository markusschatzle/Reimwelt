...
// Filled / empty circles, same visual language as the detail panel's stress
// pattern. `pattern` is a string of "1" (betont) and "0" (unbetont).
export function MeterDots({ pattern, label, className = "" }) {
  const cells = String(pattern).split("");
  const spoken = cells
    .map((c) => (c === "1" ? "betont" : "unbetont"))
    .join(" – ");
  return (
    <span
      className={`meter-dots ${className}`.trim()}
      role="img"
      aria-label={label ? `${label}: ${spoken}` : `Betonung: ${spoken}`}
    >
      {cells.map((c, i) => (
        <span
          key={i}
          className={`stress-dot ${c === "1" ? "stressed" : "unstressed"}`}
        />
      ))}
    </span>
  );
